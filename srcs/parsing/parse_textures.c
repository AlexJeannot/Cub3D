...
void parse_texture(char *str, int index)
{
  ...
  cmp_str = 2;
  cmp_path = 0;
  texture_extension(str); // Verification de l'extension du fichier (.xpm)
  ...
  if (str[cmp_str] == '.')
  {
    ...
    path[cmp_path] = '\0';
    open_texture_file(path, index); // Check ouverture du fichier et initalisation de la structure si OK
  }
  else
