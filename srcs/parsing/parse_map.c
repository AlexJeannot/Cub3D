...
void parse_map(char *str)
{
  ...
  len = ft_strlen(str);
  if (config->map == 0) // Si premiere ligne de la map
  {
    check_all_config_elem_before_map(); // Check de la présence de tous les elements
    map->map_str = ft_strjoin_cub(map->map_str, str, ft_strlen(map->map_str), ft_strlen(str));
    map->width = ft_strlen_without_space(str);
    config->map = 1;
  }
  else // Autrement join de la map avec la ligne recupérée
    map->map_str = ft_strjoin_cub(map->map_str, str, ft_strlen(map->map_str), ft_strlen(str));
  map->height++;
}

/* Transformation de la map en tableau de string */
