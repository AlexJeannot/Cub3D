...
void check_map_error(void)
{
  ...
  cmp_array = 1;
  while (cmp_array < win->my_map->height - 2)
  {
    cmp_str = 0;
    while (win->my_map->map[cmp_array][cmp_str])
    {
      ...
      else if (win->my_map->map[cmp_array][win->my_map->width - 1] != '1')
        exit_game("frontiere est non ferme\n");
      else if (win->my_map->map[cmp_array][cmp_str] == 'N' || win->my_map->map[cmp_array][cmp_str] == 'S' || win->my_map->map[cmp_array][cmp_str] == 'E' || win->my_map->map[cmp_array][cmp_str] == 'W')
